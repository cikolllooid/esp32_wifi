...
void hid_demo_task(void *pvParameters)
{
    ...
    while (1) {
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        if (sec_conn) {
            ESP_LOGI(HID_DEMO_TAG, "Processing HID commands");

            uint8_t key_value;
            uint8_t key_mask = {LEFT_CONTROL_KEY_MASK | LEFT_SHIFT_KEY_MASK}; 
            key_value = HID_KEY_ESCAPE; // Assign the value
            esp_hidd_send_keyboard_value(hid_conn_id, key_mask, &key_value, 1, 1);
            esp_hidd_send_keyboard_value(hid_conn_id, key_mask, &key_value, 0, 0);
        }
    }
}
...
void stop_hid_task() {
    if (bleTaskHandle != NULL) {
        ...
        bleTaskHandle = NULL;  // Обнуление дескриптора
        ESP_LOGI(HID_DEMO_TAG, "BLE task stopped.");
    } else {
        ESP_LOGW(HID_DEMO_TAG, "BLE task already stopped.");
    }
}
...
void start_code(int time) {
    if (ble_timer != NULL) {
        ...
        if (err == ESP_OK || err == ESP_ERR_INVALID_STATE) {
            ...
            ble_timer = NULL;  // Обнуление указателя
        } else {
            ESP_LOGE(HID_DEMO_TAG, "Error deleting timer: %s", esp_err_to_name(err));
        }
    }
    ...
    if (bleTaskHandle != NULL) {
        ESP_LOGW(HID_DEMO_TAG, "Stopping existing BLE task...");
        stop_hid_task();
    }
    ...
    if (!ble_initialized) {
        ...
        ESP_LOGW(HID_DEMO_TAG, "BLE stack already initialized.");
    }
    
    ESP_ERROR_CHECK(esp_timer_create(&timer_args, &ble_timer));
    ESP_ERROR_CHECK(esp_timer_start_once(ble_timer, time * 1000000));
    ESP_LOGI(HID_DEMO_TAG, "Timer started for %d seconds", time);

    if (bleTaskHandle == NULL) {
        xTaskCreate(&hid_demo_task, "ble_task", 4096, NULL, 3, &bleTaskHandle);
        ESP_LOGI(HID_DEMO_TAG, "BLE task started!");
    } else {
        ESP_LOGW(HID_DEMO_TAG, "BLE task already running!");
    }
}